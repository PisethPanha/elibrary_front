...
function KhmerBook() {
    ...
    function handleSearch(key) {
        ...
        setKeyword(key)
        axios.get("https://carefree-empathy-production.up.railway.app/type", { params: { keyword: key, catagory: catagory, language: "khmer" } }).then((res) => { setResult(res.data); console.log(res.data); })
    ...
    function handleSearchButton(key) {
        if (key == "" && catagory == "") {
            setBackBTN(false)
            axios.get("https://carefree-empathy-production.up.railway.app/type", { params: { keyword: "", catagory: "", language: "khmer" } }).then((res) => setData(res.data)
            )
        } else {
    ...
    function handleBackToDefaultData() {
        axios.get("https://carefree-empathy-production.up.railway.app/getbookastype", { params: { leng: "Khmer" } }).then((res) => setImage(res.data))
        setBackBTN(false)
    }
    ...
    useEffect(() => {
        axios.get("https://carefree-empathy-production.up.railway.app/getbookastype", { params: { leng: "Khmer" } }).then((res) => setImage(res.data.reverse())
        )
    }, [])
    return (
        <div>
            <div className=''>
                <br />
                <h1 className='text-[30px] font-[700] border-b-8 border-blue-500 max-w-[30rem] mx-auto text-center uppercase'>Khmer Book</h1>
                <br />
                ...
                <div className={`${backBTN ? "block" : "hidden"} w-full justify-center items-center flex p-4`}>
                    <button onClick={() => { setBackBTN(true); handleBackToDefaultData() }} className='bg-blue-600 p-2 rounded-lg text-white font-[700] '>Back</button>
                </div>
                <div className={`w-full h-full bg-transparent fixed top-0 ${dropdown ? "block" : "hidden"}`} onClick={() => setDropdown(!dropdown)}></div>
                <br />
            </div>
            <BookList img={image} more={backBTN} language="khmer" CardWidth="w-[16rem]" moreVissible={true} display={true} />
